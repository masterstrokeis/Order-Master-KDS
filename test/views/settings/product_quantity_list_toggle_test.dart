...
void main() {
  ...
  testWidgets('Product & Quantity list switch defaults on and persists off', (
    WidgetTester tester,
  ) async {
    final ProviderContainer container = await _pumpAppearance(tester);

    final Finder switchFinder = find.byKey(
      const Key('product-quantity-list-switch'),
    );
    ...
    expect(container.read(productQuantityListVisibleProvider), isFalse);
    expect(_switchValue(tester, switchFinder), isFalse);

    final ProductQuantityListPreferenceService prefs =
